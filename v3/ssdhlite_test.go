...
func TestMultipleOpen(t *testing.T) {
	...
	var repeat = func() {
		...
		rows, err := c.Query(`SELECT TOP 1 EmailKey, Subject, Format, SenderName, SenderAddress, DateQueued FROM tnfEmailOutBox;`)
		if err != nil {
			t.Log(err.Error())
			t.Fail()
			return
...
func TestQueryArray(t *testing.T) {
	...
	err = c.QueryArray(`SELECT applicationid FROM tnfEmailOutbox;`, &arr)
	if err != nil {
		t.Log(err.Error())
		t.Fail()
		return
